		...
		do
			if button = {EV_POINTER_CONSTANTS}.right then
				...
				if attached ev_mlist.selected_item then
					create an_mi.make_with_text_and_action (get_msg ({EVX_MESSAGES_IDS}.ec_remove_mi, Void), agent process_remove_existing)
				--		an_mi.set_pixmap (get_icon_pixmap ("tool/archetype_tool_new"))
					menu.extend (an_mi)
				end
				create an_mi.make_with_text_and_action (get_msg ({EVX_MESSAGES_IDS}.ec_add_mi, Void), agent process_add_new)
